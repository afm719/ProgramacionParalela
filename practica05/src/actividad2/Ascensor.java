...
public class Ascensor extends Thread
{
    ...
    private BotonLlamada boton; //boton que usara la persona
    private int capacidad; //capacidad maxima de personas en el ascensor
    
    private class BotonLlamada {
    	 ...
    	 * Detiene el ascensor 
    	 */
    	public void paraAscensor() {
    		
    		synchronized(this) {
    			while (listaAscensor.size() == 0 && contador == 0) {
        			try {
        				wait();
    ...
    public Ascensor(int capacidad)
    {
        ...
        //Crea el boton
        boton = new BotonLlamada();
        this.capacidad = capacidad; //se especifica en el contructor la capacidad
    }
	...
	public synchronized void determinaPlanta (int planta) {
		
		try {
			while (!parado || plantaActual != planta || listaAscensor.size() >= capacidad) {
				wait();
			}
		} catch (InterruptedException e) {
				return;
	...
	public synchronized void bajarPlanta (int planta) {
		
		try {
			while (!parado || plantaActual != planta) {
				wait();
			}
		} catch (InterruptedException e) {
				return;
